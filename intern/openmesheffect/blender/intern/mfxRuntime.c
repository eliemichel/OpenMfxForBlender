...
void runtime_ensure_host(OpenMeshEffectRuntime *rd)
{
  if (NULL == rd->ofx_host) {
    ...
    // Configure host
    OfxPropertySuiteV1 *propertySuite = (OfxPropertySuiteV1 *)rd->ofx_host->fetchSuite(
        rd->ofx_host->host, kOfxPropertySuite, 1);
    // Set custom callbacks
    propertySuite->propSetPointer(
        rd->ofx_host->host, kOfxHostPropBeforeMeshGetCb, 0, (void *)before_mesh_get);
    propertySuite->propSetPointer(
        rd->ofx_host->host, kOfxHostPropBeforeMeshReleaseCb, 0, (void *)before_mesh_release);
  }
}
  ...
  for (int i = 0; i < fxmd->num_parameters; ++i) {
    switch (parameters[i]->type) {
      ...
      case PARAM_TYPE_STRING:
        ...
        strncpy(parameters[i]->value[0].as_char,
                fxmd->parameter_info[i].string_value,
