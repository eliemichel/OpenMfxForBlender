...
{
  bool missing_suite = NULL == gRuntime.propertySuite || NULL == gRuntime.parameterSuite ||
...
static OfxStatus cook(OfxMeshEffectHandle instance)
{
  ...
  meshEffectSuite->meshGetAttribute(
      input_mesh, kOfxMeshAttribPoint, kOfxMeshAttribPointPosition, &pos_attrib);
  propertySuite->propGetPointer(pos_attrib, kOfxMeshAttribPropData, 0, (void **)&input_points);
  meshEffectSuite->meshGetAttribute(
      input_mesh, kOfxMeshAttribVertex, kOfxMeshAttribVertexPoint, &vertpoint_attrib);
  propertySuite->propGetPointer(
      vertpoint_attrib, kOfxMeshAttribPropData, 0, (void **)&input_vertices);
  meshEffectSuite->meshGetAttribute(
      input_mesh, kOfxMeshAttribFace, kOfxMeshAttribFaceCounts, &facecounts_attrib);
  propertySuite->propGetPointer(
      facecounts_attrib, kOfxMeshAttribPropData, 0, (void **)&input_faces);

  // Get vertex color
  OfxPropertySetHandle vcolor_attrib, uv_attrib;
  status = meshEffectSuite->meshGetAttribute(
      input_mesh, kOfxMeshAttribVertex, "color0", &vcolor_attrib);

  float *vcolor_data = NULL;
  if (kOfxStatOK == status) {
    propertySuite->propGetPointer(vcolor_attrib, kOfxMeshAttribPropData, 0, (void **)&vcolor_data);
    meshEffectSuite->attributeDefine(
        output_mesh, kOfxMeshAttribVertex, "uv0", 2, kOfxMeshAttribTypeFloat, &uv_attrib);
  }
  else {
    // DEBUG
    meshEffectSuite->attributeDefine(
  ...
  meshEffectSuite->meshGetAttribute(
      output_mesh, kOfxMeshAttribPoint, kOfxMeshAttribPointPosition, &pos_attrib);
  propertySuite->propGetPointer(pos_attrib, kOfxMeshAttribPropData, 0, (void **)&output_points);
  meshEffectSuite->meshGetAttribute(
      output_mesh, kOfxMeshAttribVertex, kOfxMeshAttribVertexPoint, &vertpoint_attrib);
  propertySuite->propGetPointer(
      vertpoint_attrib, kOfxMeshAttribPropData, 0, (void **)&output_vertices);
  meshEffectSuite->meshGetAttribute(
      output_mesh, kOfxMeshAttribFace, kOfxMeshAttribFaceCounts, &facecounts_attrib);
  propertySuite->propGetPointer(
      facecounts_attrib, kOfxMeshAttribPropData, 0, (void **)&output_faces);
...
{
  static OfxPlugin plugin = {/* pluginApi */ kOfxMeshEffectPluginApi,
